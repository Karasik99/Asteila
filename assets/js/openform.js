...
function Formcall(event){
    if((event.target.closest('.header__info-btn-1'))||(event.target.closest('.directions__card-btn'))||(event.target.closest('.offers__card-btn')) || (event.target.closest('.header__btn'))||(event.target.closest('.aside-button'))|| (event.target.closest('.footer__btn'))|| (event.target.closest('.portfolio__card-btn'))){
